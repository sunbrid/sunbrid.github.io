...
$(function () {
    $("#navGroup,#navTopGroup").on("click", ".nav_item", function () {
        ...
        //动态设置iframe框的高度
        $(".main_right_tab>.layui-tab-content>.layui-tab-item").css("height", $(".main_right.layui-body").height() - 80 + "px");
...
function findMyMenu() {
    function convertUrl(url) {
        if (url.indexOf("swagger/index.html") > 0) {
            path = requestUtilParams.host + url;
            return path;
        }
        var path = url.replace("views/", "").replace(".jsp", ".html")
        return path;
    }


    requestUtilParams.xhrGet(indexParams.action.findMyMenu).done(function (res) {
        ...
        for (var i = 0; i < menuOne.length; i++) {
            var $li = $(' <li class="layui-nav-item layui-nav-itemed menuOne_'
                ...
                + '" data-url="' + convertUrl(menuOne[i].url) + '"><i class="'
 ...
 **/
function findMyUser() {
    requestUtilParams.xhrGet(indexParams.action.findMyUser).done(function (res) {
        var user = res.data.user; //获得用户信息

        $("#session_user_name").text(user.name);
        if (res.data.fileLog != null) {
            $("#session_user_url").attr("src", requestUtilParams.convertFilePath(res.data.fileLog.save_path));
        }
    });
}
 ...
 **/
function loginOut() {
    requestUtilParams.xhrGet(indexParams.action.loginOut).done(function (res) {
